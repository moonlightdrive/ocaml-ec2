...
let put ?region bucket obj =     
  ...
  let (body, stream) = 
    let open Cohttp_lwt_body in
    let contents f = of_stream @@ Lwt_io.lines_of_file f in
    (contents obj, contents obj) in
  lwt body_str = Cohttp_lwt_body.to_string stream in
    lwt body = 
      let open Lwt_io in
      let open Lwt in
